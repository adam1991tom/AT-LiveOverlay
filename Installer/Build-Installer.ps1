...
$MsiPath = Join-Path $FinalDir 'AT-LiveOverlay-v4.2.0-Setup.msi'
...
try {
    Clear-Host
    Write-Host '============================================================'
    Write-Host '         AT LIVEOVERLAY v4.2.0 - FINAL MSI BUILDER'
    Write-Host '============================================================'
    ...
    $ReleaseInfo = [ordered]@{
        ...
        sha256 = $Hash
        silentInstall = 'msiexec.exe /i "' + [IO.Path]::GetFileName($MsiPath) + '" /qn /norestart /L*v "%ProgramData%\AT LiveOverlay\Logs\install-4.2.0.log"'
        detectionFile = 'C:\Program Files\AT LiveOverlay\ATLiveOverlay.exe'
        minimumFileVersion = '4.2.0.0'
        companionPort = 8765
