# AT LiveOverlay Companion Module v1.3.0

Development module for Bitfocus Companion. It connects to the AT LiveOverlay v4.2.0 HTTP API on TCP port 8765.
...
- **Actions**: show/hide/reload/edit/live/lock/unlock/close per overlay, set URL, set opacity, set refresh interval, set rotation (0/90/180/270°, for a monitor mounted sideways), create overlay, show/hide/reload all, and load scene.
...
- **Variables**: `version`, `build`, `overlay_count`, `active_scene`, plus per-overlay `overlay_<id>_name/url/visible/locked/opacity/refresh/rotation` that appear once that overlay exists.

## Setup

1. Install AT LiveOverlay v4.2.0 and open **Remote control** from its tray menu.
